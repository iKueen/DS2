...
// ---------- Quick Sort (iterative + median-of-three) ----------
...
void printData(const vector<int>& data) {
    cout << "[";
    int limit = min((int)data.size(), 50);
...
int main() {
    ...
    for (auto size : sizes) {
        ...
        if (size == 100) {
            cout << "\n小資料量 " << size << " 筆的各排序法執行時間 (秒):" << endl;
        }
        else if (size == 1000) {
            cout << "\n中資料量 " << size << " 筆的各排序法執行時間 (秒):" << endl;
        }
        else if (size == 10000) {
            cout << "\n大資料量 " << size << " 筆的各排序法執行時間 (秒):" << endl;
        }
